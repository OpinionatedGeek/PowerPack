...
)
{
    ...
    for (var iChildCounter = 0; iChildCounter < ctlControlToCheck.children.length; iChildCounter++)
    {
		hookUpControl_hi_opgeek (ctlControlToCheck.children [iChildCounter]);
    }
...
)
{
	...
	this.DisplaySetting = ctlControlToHide.style.display;
...
HideIf_hi_opgeek.prototype.GetValue = function (ctlCurrent)
{
	...
	else
	{
		...
		for (var iChildCounter = 0; iChildCounter < ctlCurrent.children.length; iChildCounter++)
		{
			szTemporaryValue = this.GetValue (ctlCurrent.children [iChildCounter]);
			if (szTemporaryValue != "")
			{
				szValue = szTemporaryValue;
				iChildCounter = ctlCurrent.children.length;
			}
		}
	}
...
{
	var bWeAre = false;
	if ((ctlCurrent.type != "radio") || (ctlCurrent.status == true))
	{
		bWeAre = true;
	}
