...
{
	var elButton = document.getElementById (strButtonID);
...
{
	var elCurrentCal = document.getElementById (strName + 'DatePicker_dp_opgeek');
	...
	elCurrentCal.style.position = 'absolute';
...
{
	event.currentTarget.style.backgroundColor = 'highlight';
	event.currentTarget.style.color = 'highlighttext';
...
{
	var elField = null;
	var aAllElementsWithName = document.getElementsByName (this.GetName ());
...
{
	return document.getElementById (this.GetName () + 'Display');
}

DatePicker_dp_opgeek.prototype.GetCalendar = function ()
{
	return document.getElementById (this.GetName () + 'DatePicker_dp_opgeek');
}
...
DatePicker_dp_opgeek.prototype.Show = function ()
{
	if (!this.PopupCurrentlyDisplayed)
	{
		...
		var nLeft = getLeftOffset_dp_opgeek (document.getElementById (this.GetName () + 'Icon'));
		var nTop = getTopOffset_dp_opgeek (document.getElementById (this.GetName () + 'Icon'));

		var nRemainingWidth = document.body.scrollLeft + document.body.clientWidth - document.getElementById (this.GetName () + 'Icon').offsetLeft;
		var nRemainingHeight = document.body.scrollTop + document.body.clientHeight - document.getElementById (this.GetName () + 'Icon').offsetTop - BUTTON_HEIGHT_DP_OPGEEK;
...
{
	var strHeaderHTML = '';
	strHeaderHTML += '<tr style=\'padding: 1px\'>';
	strHeaderHTML += '<td align=\'center\' width=\'26\' style=\'' + MONTH_FORWARDBACK_STYLES_DP_OPGEEK + '\' ';
	strHeaderHTML += 'onmouseover=' + QUOTE_MARK_DP_OPGEEK + 'return highlightMenuItem_dp_opgeek (event);' + QUOTE_MARK_DP_OPGEEK + ' ';
	strHeaderHTML += 'onmouseout=' + QUOTE_MARK_DP_OPGEEK + 'return unhighlightMenuItem_dp_opgeek (event);' + QUOTE_MARK_DP_OPGEEK + ' ';
	...
	strHeaderHTML += '<select id="' + this.GetName () + '_month_select" name="' + this.GetName () + '_month_select" onclick=\'return ignore_dp_opgeek (event)\' onchange=\'return monthChange_dp_opgeek (' + QUOTE_MARK_DP_OPGEEK + this.GetName () + QUOTE_MARK_DP_OPGEEK + ', ' + this.GetYear () + ')\' style="font-size: 10px;">';
	...
	strHeaderHTML += '<select id="' + this.GetName () + '_year_select" name="' + this.GetName () + '_year_select" onclick=\'return ignore_dp_opgeek (event)\' onchange=\'return yearChange_dp_opgeek (' + QUOTE_MARK_DP_OPGEEK + this.GetName () + QUOTE_MARK_DP_OPGEEK + ', ' + this.GetMonth () + ')\' style="font-size: 10px;">';
...
DatePickerPopup_dp_opgeek.prototype.GetSelectableDay = function (nDate, nMonth, nYear, nColumn)
{
	...
	strDayHTML += '\' onmouseover=' + QUOTE_MARK_DP_OPGEEK + 'return highlightMenuItem_dp_opgeek (event);' + QUOTE_MARK_DP_OPGEEK + ' ';
	strDayHTML += 'onmouseout=' + QUOTE_MARK_DP_OPGEEK + 'return unhighlightMenuItem_dp_opgeek (event);' + QUOTE_MARK_DP_OPGEEK + ' ';
...
DatePickerPopup_dp_opgeek.prototype.GetDaysInMonthTable = function ()
{
	...
	if (nColumn != 0)
	{
		for (nCounter = 1; nColumn != 0; nCounter++)
		{
...
DatePickerPopup_dp_opgeek.prototype.GetCalendarPopup = function ()
{
	...
	strPopupHTML += '<table width=\'182\' id=\'' + this.GetName () + 'CalendarTable\' name=\'' + this.GetName () + 'CalendarTable\' cellpadding=\'0\' cellspacing=\'0\' ';
	strPopupHTML += 'style=\'' + POPUP_FONT_STYLES_DP_OPGEEK + ' background: #E0E0E0; border-width: 2px; border-style: outset; border-color: buttonhighlight;\'>';
	strPopupHTML += this.GetCalendarHeader (dtWorking);
	if (this.GetExtruded ())
	{
		strPopupHTML += '<tr style=\'padding: 1px\'><td colspan=\'7\' style=\'padding: 1px\'><table border=\'0\' width=\'100%\' style=\'' + POPUP_FONT_STYLES_DP_OPGEEK + ' background: #E0E0E0; border-width: 2px; border-style: outset; border-color: buttonhighlight;\' id=\'' + this.GetName () + 'CalendarBodyTable\' name=\'' + this.GetName () + 'CalendarBodyTable\'>';
	}
