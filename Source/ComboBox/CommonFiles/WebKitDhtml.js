...
{
	var insCurrentInstance = getInstantiatedFieldByTextboxId_cb_opgeek (szComboId);
	insCurrentInstance.rawDropdownField.onchange ();
...
ComboBox_cb_opgeek.prototype.BlurTextbox = function ()
{
	if (this.userEdited)
	{
		...
		this.rawDropdownField.focus ();
		this.rawDropdownField.blur ();
	}
...
ComboBox_cb_opgeek.prototype.Layout = function ()
{
	if (this.rawTextField.offsetWidth > 0)
	{
		...
		var editableBodyWidth = (controlWidth - DROPDOWN_DOWN_ARROW_WIDTH) + 'px';
		var boundingWidth = (controlWidth) + 'px';
		if (!this.textboxShrunk)
		{
			this.rawTextField.style.width = editableBodyWidth;
			this.textboxShrunk = true;
		}

		this.rawDropdownField.style.width = boundingWidth;
		this.rawDropdownField.style.height = this.rawTextField.offsetHeight;

		// Set the bounding box to be the right width.
		this.rawTextField.offsetParent.style.width = boundingWidth;

		this.rawDropdownField.style.display = '';

		var szClippingRegion = 'rect(auto auto auto ' + editableBodyWidth + ')';
	...
	else
	{
		if (this.enteredText.length > 0)
		{
			...
			if ((nFoundOption == true) && (eventArgs.keyCode != 46) && (eventArgs.keyCode != 8))
			{
				...
				this.rawTextField.setSelectionRange (this.enteredText.length, this.rawTextField.value.length);
			}
			else
			{
				this.UpdateChild ();
