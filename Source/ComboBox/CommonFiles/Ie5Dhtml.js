...
)
{
	...
	{
		insCurrentInstance.rawTextField.fireEvent ('onchange');
	}
...
ComboBox_cb_opgeek.prototype.BlurTextbox = function ()
{
	if (this.userEdited)
	{
		window.setTimeout ("fireOnChangeEventSafely_cb_opgeek ('" + this.id + "')", 100);
...
ComboBox_cb_opgeek.prototype.Layout = function ()
{
	...
	{
		this.rawDropdownField.style.width = this.rawTextField.offsetWidth;
		// We'd like to do this, but IE won't let us change the height of a dropdown.
		// this.rawDropdownField.style.height = this.rawTextField.offsetHeight;
		// Unfortunately we can't even do this to force the textbox to be the right height:
		// this.rawTextField.style.height = this.rawDropdownField.offsetHeight;
		this.rawDropdownField.style.left = getLeftOffset_cb_opgeek (this.rawTextField);
		this.rawDropdownField.style.top = getTopOffset_cb_opgeek (this.rawTextField);

		this.rawDropdownField.style.display = "";

		var szClippingRegion = 'rect (auto auto auto ' + (this.rawDropdownField.clientWidth - 20) + ')';
